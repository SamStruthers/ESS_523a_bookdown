...
```{r}
unpack_nps_visits <- function(year){
...
visits <- map(years, unpack_nps_visits)%>%
list_rbind()
...
```{r}
raw_romo_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=1&startYear=2010&endMonth=12&endYear=2021")
romo_visit_data <- httr::content(raw_romo_data, as = "text", encoding = "UTF-8")
romo_final_data <- jsonlite::fromJSON(romo_visit_data)
...
```{r}
unit_visitation <- function(start_date, end_date, park_code){
...
#tester
romo <- unit_visitation("1990-01-01","2021-12-01", "ROMO" )
ever <- unit_visitation("1990-01-01","2021-12-01", "EVER" )
thro <- unit_visitation("1990-01-01","2021-12-01", "THRO" )

all_sites <- rbind(romo, ever, thro)
...
site_list <- c("ROMO", "EVER", "THRO")
unit_total<- vector("list", length = length(site_list))

for (i in 1:length(site_list)) {
  
  unit_visit <- unit_visitation("1990-01-01","2021-12-01", site_list[i] )
  unit_total <- rbind(unit_visit, unit_total)
...
output_visits <- site_list %>% 
  map(~ unit_visitation(start_date = "1990-01-01",end_date = "2021-12-01", park_code = .))%>%
  list_rbind()


identical(output_visits , all_sites)
