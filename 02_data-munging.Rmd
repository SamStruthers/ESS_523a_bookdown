...
```{r}
unit_visitation_1980_2021 <- function( park_code){
  ...
  raw_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", park_code, "&startMonth=",start_month,"&startYear=",start_year,"&endMonth=",end_month,"&endYear=", end_year ))
content_data <- httr::content(raw_data, as = "text", encoding = "UTF-8")
final_data <- jsonlite::fromJSON(content_data)
  return(final_data)
} 

site_list <- c("ROMO", "ACAD", "LAKE", "YELL", "GRCA", "ZION", "OLYM", "GRSM")

units <- site_list %>% 
  map(~ unit_visitation_1980_2021( park_code = .))%>%
  list_rbind()
...
```{r}
yearly_park_parkwide <- yearly %>%
  mutate(type = ifelse(UnitCode == "Parkwide", "Parkwide", "Unit"))

yearly_park_parkwide_plot<-  ggplot(data=yearly_park_parkwide) +
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()) +
...
ggplotly(yearly_park_parkwide_plot)

yearly <- yearly_park_parkwide
```
...
perc_of_total <- wide_data%>%
  mutate_at(.vars = site_list, .funs = ~ (./Parkwide)*100 )%>%
  pivot_longer(cols = -c(Parkwide, Year), names_to = "park_code", values_to = "percent")
 

percent_plot <- ggplot(data = perc_of_total)+
  geom_line(aes(x = Year, y = percent, color = park_code))+
  theme_bw(base_size = 10)

ggplotly(percent_plot)
```
...
```{r}
joined_percent <- joined_data%>%
  group_by(UnitCode.x, Year)%>%
  summarise(
    all_visits = sum(RecreationVisitors.y),
    park_visits = sum(RecreationVisitors.x),
  )%>%
  mutate(percent_visits = (park_visits/all_visits)*100, 
         park_code = UnitCode.x)

percent_plot2 <- ggplot(data = joined_percent)+
  geom_line(aes(x = Year, y = percent_visits, color = park_code))+
  theme_bw(base_size = 10)

ggplotly(percent_plot2)
```
...
avg_visits <- yearly%>%
  filter(Year >=1990)%>%
  group_by(UnitCode)%>%
  summarise(avg_per_year_visits = mean(RecVisitation), 
            max_per_year_visits = max(RecVisitation), 
            min_per_year_visits = min(RecVisitation))

view(avg_visits)

##GRSM has the highest avg visits per year out of any park
